...
class Vex(object):
    """
    A class to parse and extract information from VEX (VLBI Experiment) files.

    Parameters
    ----------
    filename : str
        The path to the VEX file to be parsed.
    fill_array : bool, optional
        If True, constructs an Array object with site information (default is False).

    Attributes
    ----------
    filename : str
        The path to the VEX file.
    metalist : list
        List of metadata sectors divided by '$' marks.
    source : list
    ...
    bw_hz : float
        Bandwidth in Hz.
    array : ehtim.array.Array or None
        Array object containing site information if `fill_array` is True, otherwise None.
    sched : list
    ...
    get_variable(vname, line)
        Get the value of 'vname' in a line.
    find_variable(vname, line)
        Find a variable 'vname' in a line.
    get_SEFD(station)
        Find SEFD for a given station.
    get_obs_timerange(source)
        Find the time range that any station observes the source.
    """

    def __init__(self, filename, fill_array=False):
        ...
        # import ehtim only if array is to be filled; this is a hack to avoid long import times for ehtim
        if fill_array:
            import ehtim.array
